...
class TestOpinelServicesIAM:
    ...
    def test_001_create_user(self):
        user_data = create_user(self.api_client, 'OpinelUnitTest001')
        assert len(user_data['errors']) == 0
        user_data = create_user(self.api_client, 'OpinelUnitTest001')
        assert len(user_data['errors']) == 1
        user_data = create_user(self.api_client, 'OpinelUnitTest002', 'BlockedUsers')
        assert len(user_data['errors']) == 0
        user_data = create_user(self.api_client, 'OpinelUnitTest003', ['BlockedUsers', 'AllUsers'])
        assert len(user_data['errors']) == 1
        user_data = create_user(self.api_client, 'OpinelUnitTest004', with_password = True)
        assert len(user_data['errors']) == 0
        assert 'password' in user_data
        assert len(user_data['password']) == 16
        user_data = create_user(self.api_client, 'OpinelUnitTest005', with_password=True ,require_password_reset = True)
        assert len(user_data['errors']) == 0
        assert 'password' in user_data
        assert len(user_data['password']) == 16
        user_data = create_user(self.api_client, 'OpinelUnitTest006', with_access_key = True)
        assert len(user_data['errors']) == 0
    ...
    def test_002_add_user_to_group(self):
        create_user(self.api_client, 'OpinelUnitTest010')
        create_user(self.api_client, 'OpinelUnitTest011')
        add_user_to_group(self.api_client, 'OpinelUnitTest010', 'BlockedUsers', True)
        add_user_to_group(self.api_client, 'OpinelUnitTest011', 'BlockedUsers', False)
    ...
    def test_004_get_access_keys(self):
        create_user(self.api_client, 'OpinelUnitTest020', with_access_key = True)
        access_keys = get_access_keys(self.api_client, 'OpinelUnitTest020')
        assert len(access_keys) == 1


    def test_005_show_access_keys(self):
        show_access_keys(self.api_client, 'OpinelUnitTest020')
    ...
    def test_999_delete_user(self):
        ...
        while True:
            ...
            remaining_users = []
            for user in users:
                errors = delete_user(self.api_client, user)
                if len(errors):
